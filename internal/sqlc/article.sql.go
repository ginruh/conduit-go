...
FROM article AS a
     ...
     ON uf.follower_id = a.author_id AND uf.user_id = $1
     LEFT JOIN
        (
     ...
     ON a.id = af_favorited.article_id AND af_favorited.user_id = $1
     LEFT JOIN
     (
     ...
     ON
        ...
        CASE
            ...
            ELSE af_list.username IN(SELECT unnest(string_to_array($2, ',')) as parts)
        END
     LEFT JOIN
     (
         ...
         WHERE
             CASE
                 ...
                 ELSE at.tag_name IN(SELECT unnest(string_to_array($3, ',')) as parts)
             END
         GROUP BY at.article_id
     ) AS at
    ON a.id = at.article_id AND at.article_id = a.id
WHERE
    $4 IS NULL OR u.username IN(SELECT unnest(string_to_array($4, ',')) as parts)
  AND
    at.tags is not null
...
func (q *Queries) ListArticles(ctx context.Context, arg ListArticlesParams) ([]ListArticlesRow, error) {
	rows, err := q.db.QueryContext(ctx, listArticles, arg.CurrentUser, arg.Offset, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
